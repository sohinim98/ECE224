    ...
    for (;;) {
        ...
        case 'f':              /* FatFS API controls */
            ...
            case 'p':          /* fp <len> - read and play file from current fp */
            	...
            	int rate = 2;
            	int stereo = 1;
            	volatile unsigned int settings = IORD(SWITCH_PIO_BASE, 0) & 0x03;
            	switch(settings)
            	{
            	case 0x00:
            		xprintf("Normal Speed\n");
            		break;
            	case 0x01:
            		xprintf("Half speed\n");
            		rate = 1;
                ...
                // number of bytes read != number of bytes we wanted to read
                while (p1 > 0) {
                	// code goes here
                	// clear s2 that stores number of bytes successfully read
                	s2 = 0;
                	cnt = (uint32_t)p1 >= blen ? blen : p1;
                	p1 =  p1 - blen;
					...
					if (cnt != s2) //error if cnt does not equal s2 referenced value ???
						break;
					// End read file into buffer

					i = 0;
                	// s2 has the number of file bytes read
                	for(i = 0; i < s2; i += rate*2){
                		l_buf = 0;
                		r_buf = 0;
